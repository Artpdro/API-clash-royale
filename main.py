...
class ClashRoyaleAnalytics:
            ...
            {"$match": {
                ...
                "$expr": {
                    "$and": [
                        {"$eq": ["$winner", "$playerTag"]},
                        {"$lte": [
                            # Corrigido: Acessa o primeiro elemento do array e converte para inteiro
                            {"$multiply": [
    ...
    def run_query5(self):
        ...
        pipeline = [
            {"$match": {
                ...
                "$expr": {"$eq": [{"$size": "$team.cards.id"}, 8]}  # Só decks completos
            }},
    ...
    def run_query6(self):
        ...
        pipeline = [
            {"$match": {
                "team.startingTrophies": {"$gte": min_trophies}  # Ajuste do caminho do campo
            }},
            {"$unwind": "$team"},
            {"$unwind": "$team.cards"},
            {"$group": {
